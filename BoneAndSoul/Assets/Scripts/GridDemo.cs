...
public class GridDemo : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        if (drawGrid)
        {
            Gizmos.DrawWireCube(transform.position, new Vector3(gridWorldSize.x, gridHigh, gridWorldSize.y));

            if (grid != null)
            {
                foreach (NodeDemo n in grid)
                {
    ...
    public NodeDemo NodeFromWorldPoint(Vector3 worldPosition)
    {
